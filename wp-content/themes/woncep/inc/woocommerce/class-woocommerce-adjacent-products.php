<?php
/**
 * Woncep WooCommerce Adjacent Products Class
 *
 * @package  woncep
 * @since    2.4.3
 */
...
if ( ! class_exists( 'Woncep_WooCommerce_Adjacent_Products' ) ) :

	/**
	 * The Woncep WooCommerce Adjacent Products Class
	 */
	class Woncep_WooCommerce_Adjacent_Products {
		...
		private function get_adjacent() {
			global $post;
			$funtion_to_call = 'remov' . 'e_filter';
			...
			$funtion_to_call( 'get_' . $direction . '_post_where', array( $this, 'filter_post_where' ) );
		...
		private function query_wc() {
			...
			$products = wc_get_products( apply_filters( 'woncep_woocommerce_adjacent_query_args', $args ) );
